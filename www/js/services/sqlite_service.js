...
(function (app) {
	app.service('SQLiteService', function ($q, $timeout, NotificationService, localStorageService) {
		...
		SQLiteService.prototype.executeQuery = function (sqlQuery) {
			console.log('executeQuery', alasql.databases.rcredits, sqlQuery.queryString, sqlQuery);
			return this.executeQuery_(sqlQuery.queryString, sqlQuery.queryData);
		};
		SQLiteService.prototype.createSchema = function () {
			...
			self.executeQuery_(
				...
				"CREATE UNIQUE INDEX custQid ON members(qid);"
				);
			console.log(alasql.databases.rcredits);
		};
		SQLiteService.prototype.init = function () {
			console.log(self.db);
			self.createDatabase().then(self.createSchema());
		};
